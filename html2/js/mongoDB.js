...
aMongoIntel.service('mongoDB',
        function($http, $timeout, $location, $rootScope) {
    var me = this; // Let's us get easy access inside async callbacks
    // where this has changed
    this.pageSize = 10;
    this.currentQuery = 'test';
    this.lastUpdate = [ 1 ]; // Hack to get Angular to re render some
    // things
    ...
    $http.get('/searches').success(function(data) {
    ...
    this.getCurrentResultsPage = function() {
        // Want to get HTML parameters
        var params = $location.search();
        me.queryTerms = params['terms'];
        me.queryID = params['search']

        if (params['recordno']) {

            me.currentRecordNo = params['recordno']
        } else {

            me.currentRecordNo = 0;
        }

        var start = Math.floor(me.currentRecordNo / me.pageSize)
        * me.pageSize;
        var end = start + me.pageSize;

        console.log("Check cache start=" + start + " rs="
                + me.resultStart)
        ...
        // Do we have the info for the cache? - if not we should fetch
        // it
        ...
        if (start >= me.resultSet.length) {
            $http({
                ...
                method : "GET",
                params : {
                    queryID : me.searchList[me.queryID].name,
                    start : start,
                    page : me.pageSize,
                    terms : me.queryTerms
                }
    ...
    // This is a key function as it actually does the searches to get
    // the data

    this.getCurrentResultsPageOld = function() {
    ...
    this.nextPage = function() {
        var newRecNo = me.currentRecordNo
        - (me.currentRecordNo % me.pageSize) + me.pageSize
